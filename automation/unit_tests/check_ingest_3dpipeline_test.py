...
class CheckIngest3dPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        db.create_tile_3d_pipeline_tables()
        ...
        for row in _3d_data:
            db.insert_3d_pipeline_test_data(row[0], row[1], row[2], row[3])
    ...
    def test_get_tiles_for_ingest(self):
        ...
        """
        tiles_to_run = check_ingest_3Dpipeline.get_tiles_for_ingest('1')
    ...
    def test_update_status(self):
        ...
        """
        num_rows = check_ingest_3Dpipeline.update_status(tile_number=1235, band='943MHz', status='Ingested')
        assert num_rows == 1 #succesful update
		#Check if the value was updated
        results = db_query.get_3d_tile_for_tests(1235, '1')
        assert len(results) == 1
        assert results[0][3] == 'Ingested'

        num_rows = check_ingest_3Dpipeline.update_status(tile_number=000, band='943MHz', status='Ingested')
